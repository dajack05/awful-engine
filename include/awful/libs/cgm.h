...
#define CgmVec2_from_f16(v)                                                    \
  (CgmVec2) { CgmF16_to_float((v).x), CgmF16_to_float((v).y) }
#define CgmVec2_to_f16(v)                                                      \
  (CgmVec2F16) { CgmF16_from_float((v).x), CgmF16_from_float((v).y) }
#define CgmVec2F16_init(x, y)                                                  \
  (CgmVec2F16) { CgmF16_from_float(x), CgmF16_from_float(y) }
#define CgmVec2_zero                                                           \
  (CgmVec2) { 0 }
#define CgmVec2_inf                                                            \
  (CgmVec2) { INFINITY, INFINITY }
#define CgmVec2_neg_inf                                                        \
  (CgmVec2) { -INFINITY, -INFINITY }
...
// absorbs all the force in @param(v) that that goes in the opposite direction
// as @param(normal)
...
#define CgmVec4_init(x_, y_, z_, w_)                                           \
  ((CgmVec4){.x = x_, .y = y_, .z = z_, .w = w_})
...
#define CgmAabb2d_init(x_, y_, ex_, ey_)                                       \
  (CgmAabb2d) { .x = x_, .y = y_, .ex = ex_, .ey = ey_ }
#define CgmAabb2d_init_wh(x_, y_, w, h)                                        \
  (CgmAabb2d) { .x = x_, .y = y_, .ex = x_ + w, .ey = y_ + h }
...
#define cgm_2d_ray_vs_aabb(a, r, r_max_distance, contact_normal_out,           \
                           contact_distance_out)                               \
  cgm_2d_aabb_vs_ray(r, r_max_distance, a, contact_normal_out,                 \
                     contact_distance_out)

CgmBool cgm_2d_swept_aabb_vs_aabb(CgmAabb2d *a, CgmVec2 a_dir, float a_distance,
                                  CgmAabb2d *b, CgmVec2 *contact_normal_out,
                                  float *contact_distance_out);
...
CgmBool cgm_2d_aabb_vs_pt(CgmAabb2d *a, CgmVec2 pt, CgmVec2 *contact_normal_out,
                          float *contact_distance_out);
#define cgm_2d_pt_vs_aabb(pt, a, contact_normal_out, contact_distance_out)     \
  cgm_2d_aabb_vs_pt(a, pt, contact_normal_out, contact_distance_out)
...
#define CgmAabb3d_init(x_, y_, z_, ex_, ey_, ez_)                              \
  ((CgmAabb3d){                                                                \
      .x = (x_), .y = (y_), .z = (z_), .ex = (ex_), .ey = (ey_), .ez = (ez_)})
#define CgmAabb3d_init_min_max(min_, max_)                                     \
  ((CgmAabb3d){.min = (min_), .max = (max_)})
...
//
CgmBool cgm_3d_ray_vs_aabb(CgmRay3d *ray, float ray_max_contact_distance,
                           CgmAabb3d *aabb, CgmVec3 *contact_normal_out,
                           float *contact_distance_out);
